...
def test_build_sales_context_matches_fixture(case_id: str) -> None:
    project_fact = run_minimal_runtime_chain(scenario_id=case_id, requested_at="2026-04-05T10:00:00+08:00")["stage6"]["project_fact"]
    payload = build_sales_context(project_fact)
    assert validate_schema("docs/contracts/schemas/stage7_sales_context.schema.json", payload) == []
    assert payload["coverage_sellable_state"] == project_fact["coverage_sellable_state"]
    assert payload["award_suspicion_summary"] == project_fact["award_suspicion_summary"]
    assert payload["tender_fairness_risk"] == project_fact["tender_fairness_risk"]
