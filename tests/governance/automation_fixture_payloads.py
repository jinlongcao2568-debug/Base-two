...
def _coordinator_refresh_intent() -> dict[str, Any]:
    return {
        ...
        "command_argv": ["python", "scripts/roadmap_candidate_maintainer.py", "--loop", "--interval-seconds", "60"],
        "examples": ["协调窗口：定时更新候选池"],
        "action_any": ["更新", "刷新", "定时更新"],
        "context_any": ["协调窗口", "候选池"],
        "disallow_any": ["当前窗口", "恢复"],
    }


def _task_window_loop_intent() -> dict[str, Any]:
    return {
        ...
        "command_argv": ["python", "scripts/worker_self_loop.py", "loop", "--interval-seconds", "60"],
        "examples": ["任务窗口：领取任务执行完毕持续领取持续开发", "任务窗口：持续干活"],
        "action_any": ["领取任务", "持续领取", "持续开发", "持续干活"],
        "context_any": ["任务窗口", "当前窗口", "worker", "clone"],
        "disallow_any": ["路线图", "候选池", "审查窗口"],
    }


def _review_window_intent() -> dict[str, Any]:
    return {
        ...
        "examples": ["审查窗口：检查候选池和未完成任务"],
        "action_any": ["审查", "检查", "排查"],
        "context_any": ["审查窗口", "候选池", "未完成任务", "不兼容", "执行一半"],
        "disallow_any": ["领取任务", "持续开发"],
    }


def _recover_intent() -> dict[str, Any]:
    return {
        ...
        "examples": ["恢复：TASK-RM-STAGE1-CORE-CONTRACT"],
    }


def _coordination_and_publish_intents() -> list[dict[str, Any]]:
    return [
        {
...
def automation_intents_payload() -> dict[str, Any]:
    return {
        ...
        "supported_intents": [
            _coordinator_refresh_intent(),
            _task_window_loop_intent(),
            _review_window_intent(),
            _recover_intent(),
            *_coordination_and_publish_intents(),
        ],
    }
