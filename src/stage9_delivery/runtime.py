...
def _delivery_readiness(project_fact: dict[str, Any]) -> str:
    ...
    if sale_gate_status == "OPEN" and report_status in {"READY", "ISSUED"}:
        return "ready"
    if sale_gate_status == "REVIEW":
        return "review_pending"
    if sale_gate_status == "HOLD":
        return "on_hold"
    return "blocked"
...
def build_delivery_payload(
    ...
    return {
        ...
        "payload_summary": (
            f"Delivery payload is {delivery_readiness} based on sale gate "
            f"{project_fact['sale_gate_status']} and report status {project_fact['report_status']}."
        ),
    }
