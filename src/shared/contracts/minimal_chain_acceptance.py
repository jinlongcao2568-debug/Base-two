...
PLAYBOOK_PATH = "docs/governance/EXECUTION_PLAYBOOKS.yaml"
POLICY_PATH = "docs/governance/ACCEPTANCE_POLICY.yaml"


def _load_playbook() -> dict[str, Any]:
    document = load_yaml(PLAYBOOK_PATH)
    return document["playbooks"]["minimal_runtime_chain_v1"]


def _load_policy() -> dict[str, Any]:
    document = load_yaml(POLICY_PATH)
    return document["policies"]["minimal_runtime_chain_v1"]
...
def evaluate_minimal_chain_acceptance(output_paths: dict[str, str], scenario_id: str) -> dict[str, Any]:
    ...
    for artifact_key in playbook["artifacts"]:
        if artifact_key not in output_paths:
            errors.append(f"missing artifact from output map: {artifact_key}")

    for artifact_key, schema_path in policy["schema_checks"].items():
    ...
    for artifact_key, fixture_pattern in policy["fixture_checks"].items():
        payload = load_json(output_paths[artifact_key])
    ...
    whitelist = load_yaml(policy["consumer_field_whitelist"])
    allowed_fields = {
        item["field"]
