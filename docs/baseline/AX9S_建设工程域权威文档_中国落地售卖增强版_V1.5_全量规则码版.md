...
5. 当前基线中的画像对象和风险字段，必须先进入字段策略字典，才允许进入页面 / API / 导出。
...
- 阶段 4 是唯一正式出结果阶段；
- 阶段 6 是唯一统一主判断中枢；
- 线索、观察、请求核验与终局定性之间必须保持边界；
- “已实现”“已验证”“已可售”“已可交付”必须分别表达，不得互相偷换。

#### 对实现的要求

- 阶段 2 必须补齐对应抓取字段；
- 阶段 3 必须补齐对应结构化对象；
- 阶段 4 必须补齐规则码与 `review_request` 模板；
- 阶段 6 必须补齐聚合字段；
- 页面、接口、测试、导出、复核链路与覆盖治理注册表必须同步对齐。

#### 对齐风险提示

- `breaking_change`：YES
- 触发条件：
  1. `project_fact` 治理侧消费字段与聚合约束未同步；
  2. 覆盖治理状态与交付阻断状态未纳入正式消费字段；
  3. 客户交付、外发 API、导出模板与对象级交付矩阵未统一进入门禁层；
  4. 新增专题仍依赖局部页面或接口拼装，而未完成阶段 2、阶段 3、阶段 4、阶段 6 的正式落库。
- 主要影响范围：对象契约、页面消费、外发导出、可售治理、发布门禁、测试回归。
